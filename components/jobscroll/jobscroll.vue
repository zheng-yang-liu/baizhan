<template>
	<view class="job_scroll_box">
		<scroll-view scroll-x="true" class="job_scroll_con_box"><!-- 左右滚动 -->
			<view class="job_scroll_con">
				<view class="job_scroll_info" v-for="(item,index) in list" :key="index"><!-- 循环list数组里面的数据 -->
					<!-- :class="item.colors"放在前后端都行放在后端前端的页面结构会容易很多 -->
					<view class="job_scroll_card" :class="item.colors"><!-- 动态class适配颜色值   -->
						<view class="job_scroll_card_T">{{ item.textT }}</view><!-- 放置主文本信息 -->
						<view class="job_scroll_card_icon">
							<view class="icon iconfont" :class="item.icon"></view><!-- 循环图标 -->
						</view>
						<view class="job_scroll_card_des">{{ item.text }}</view><!-- 卡片描述文本 -->
						<view class="job_scroll_card_btn">免费试学</view><!-- 免费试学按钮 -->
					</view>
				</view>
			</view>
		</scroll-view>
